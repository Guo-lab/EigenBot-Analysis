...
if __name__ == "__main__":
    ...
    time_bins = np.linspace(
        0, window_length_seconds, num=window_length_seconds * 8
    )  # 8 / 64 s
    ...
    for (
        ...
        for time, phase in zip(time_front, window_front):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_front[bin_index] += 1
        for time, phase in zip(time_middle, window_middle):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_middle[bin_index] += 1
        for time, phase in zip(time_hind, window_hind):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_hind[bin_index] += 1
        for time, phase in zip(time_front_2, window_front_2):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_front_2[bin_index] += 1
        for time, phase in zip(time_middle_2, window_middle_2):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_middle_2[bin_index] += 1
        for time, phase in zip(time_hind_2, window_hind_2):
            if phase == 0:
                bin_index = np.digitize(time, time_bins) - 1
                swing_counts_hind_2[bin_index] += 1

        swing_counts_front[swing_counts_front != 0] = 1
        swing_counts_middle[swing_counts_middle != 0] = 1
        swing_counts_hind[swing_counts_hind != 0] = 1
        swing_counts_front_2[swing_counts_front_2 != 0] = 1
        swing_counts_middle_2[swing_counts_middle_2 != 0] = 1
        swing_counts_hind_2[swing_counts_hind_2 != 0] = 1

        swing_counts_front_stack = np.vstack(
            (swing_counts_front_stack, swing_counts_front)
        )
        swing_counts_middle_stack = np.vstack(
            (swing_counts_middle_stack, swing_counts_middle)
        )
        swing_counts_hind_stack = np.vstack(
            (swing_counts_hind_stack, swing_counts_hind)
        )
        swing_counts_front_2_stack = np.vstack(
            (swing_counts_front_2_stack, swing_counts_front_2)
        )
        swing_counts_middle_2_stack = np.vstack(
            (swing_counts_middle_2_stack, swing_counts_middle_2)
        )
        swing_counts_hind_2_stack = np.vstack(
            (swing_counts_hind_2_stack, swing_counts_hind_2)
        )
    ...
    print(swing_counts_front_stack)
    swing_counts_front = np.mean(swing_counts_front_stack, axis=0)
    swing_counts_middle = np.mean(swing_counts_middle_stack, axis=0)
    swing_counts_hind = np.mean(swing_counts_hind_stack, axis=0)
    swing_counts_front_2 = np.mean(swing_counts_front_2_stack, axis=0)
    swing_counts_middle_2 = np.mean(swing_counts_middle_2_stack, axis=0)
    swing_counts_hind_2 = np.mean(swing_counts_hind_2_stack, axis=0)
